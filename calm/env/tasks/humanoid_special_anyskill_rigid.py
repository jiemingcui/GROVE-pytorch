...
class HumanoidSpecAnySKillRigid(HumanoidAMPGetup):
    ...
    def _reset_target(self, env_ids):
        ...
        # # soccerball
        # self._target_states[env_ids, 0] = 0.6
        # self._target_states[env_ids, 1] = 0
        # self._target_states[env_ids, 2] = 0.3
        # init_rot_theta = 0 * np.pi * torch.ones([n], dtype=self._target_states.dtype, device=self._target_states.device)
        ...
        # # pillar
        # self._target_states[env_ids, 0] = 1
        # self._target_states[env_ids, 1] = 0
        # self._target_states[env_ids, 2] = 0.9
        # init_rot_theta = 1 * np.pi * torch.ones([n], dtype=self._target_states.dtype, device=self._target_states.device)
        # # init_rot_theta = np.pi * torch.ones([n], dtype=self._target_states.dtype, device=self._target_states.device)
        # axis = torch.tensor([0.0, 0.0, 1.0], dtype=self._target_states.dtype, device=self._target_states.device)
        # init_rot = quat_from_angle_axis(init_rot_theta, axis)
        ...
        # # yard
        # self._target_states[env_ids, 0] = -10
        # self._target_states[env_ids, 1] = 8
        # self._target_states[env_ids, 2] = 0.35
        # init_rot_theta = 0 * np.pi * torch.ones([n], dtype=self._target_states.dtype, device=self._target_states.device)
        # # init_rot_theta = np.pi * torch.ones([n], dtype=self._target_states.dtype, device=self._target_states.device)
        # axis = torch.tensor([1.0, 0.0, 0.0], dtype=self._target_states.dtype, device=self._target_states.device)
        # init_rot = quat_from_angle_axis(init_rot_theta, axis)
        ...
        # # yard2
        # self._target_states[env_ids, 0] = -12.5
        # self._target_states[env_ids, 1] = 9.5
        ...
        # # stair
        # self._target_states[env_ids, 0] = -0.8
        # self._target_states[env_ids, 1] = -0.2
        # self._target_states[env_ids, 2] = 0
        # init_rot_theta = 0.5 * np.pi * torch.ones([n], dtype=self._target_states.dtype, device=self._target_states.device)
        # # init_rot_theta = np.pi * torch.ones([n], dtype=self._target_states.dtype, device=self._target_states.device)
        # axis = torch.tensor([1.0, 0.0, 0.0], dtype=self._target_states.dtype, device=self._target_states.device)
        ...
        # table
        self._target_states[env_ids, 0] = 0
        self._target_states[env_ids, 1] = 1
        self._target_states[env_ids, 2] = 0
        init_rot_theta = 0.5 * np.pi * torch.ones([n], dtype=self._target_states.dtype, device=self._target_states.device)
        # init_rot_theta = np.pi * torch.ones([n], dtype=self._target_states.dtype, device=self._target_states.device)
        axis = torch.tensor([1.0, 0.0, 0.0], dtype=self._target_states.dtype, device=self._target_states.device)
        ...
        # # living room
        # self._target_states[env_ids, 0] = -4
        # self._target_states[env_ids, 1] = -2
        # self._target_states[env_ids, 2] = -0.06
        # init_rot_theta = 0.5 * np.pi * torch.ones([n], dtype=self._target_states.dtype, device=self._target_states.device)
        # # init_rot_theta = np.pi * torch.ones([n], dtype=self._target_states.dtype, device=self._target_states.device)
        # axis = torch.tensor([1.0, 0.0, 0.0], dtype=self._target_states.dtype, device=self._target_states.device)
        # init_rot = quat_from_angle_axis(init_rot_theta, axis)
    ...
    def _load_target_asset(self):
        asset_root = "calm/data/assets/mjcf/new/table/"
        # asset_file = "in.urdf"
        # asset_file = "yard.urdf"

        # asset_file = "chair.urdf"
        # asset_file = "soccerball.urdf"
        # asset_file = "pillar.urdf"

        # asset_file = "stairs.urdf"
        # asset_file = "couch.urdf"
