...
class HumanoidSpecAnySKillArti(HumanoidAMPGetup):
    ...
    def _reset_target(self, env_ids):
        n = len(env_ids)
        init_dist = 0.7 * torch.ones([n], dtype=self._target_states.dtype, device=self._target_states.device)
        init_theta = 0.5 * np.pi * torch.ones([n], dtype=self._target_states.dtype,
                                              device=self._target_states.device)
        self._target_states[env_ids, 0] = init_dist * torch.cos(init_theta) + self._initial_humanoid_root_states[
            env_ids, 0] - 0.5
        self._target_states[env_ids, 1] = init_dist * torch.sin(init_theta) + self._initial_humanoid_root_states[
            env_ids, 1]
        self._target_states[env_ids, 2] = 0.0

        init_rot_theta = 0.3 * np.pi * torch.ones([n], dtype=self._target_states.dtype, device=self._target_states.device)
        ...
        self.gym.set_dof_state_tensor_indexed(self.sim,
                                              ...
                                              gymtorch.unwrap_tensor(huamnoids_env_ids_int32),
                                              len(huamnoids_env_ids_int32))
    ...
    def _build_target(self, env_id, env_ptr):
        ...
        target_handle_options = self.gym.get_actor_dof_properties(env_ptr, target_handle)
        target_handle_options['driveMode'].fill(gymapi.DOF_MODE_EFFORT)
        self.gym.set_actor_dof_properties(env_ptr, target_handle, target_handle_options)
    ...
    def __del__(self):
        if self.save_inference_motion:
            ...
            np.save("./output/motions/interarti.npy", self.f)
    ...
    def post_physics_step(self):
        ...
        if self.save_inference_motion:
            data = self._rigid_body_state.view(self.num_envs, self._rigid_body_state.shape[0] // self.num_envs,
                                               13).cpu().numpy()
